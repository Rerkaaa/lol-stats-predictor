...
async function valorantConfirmedLineupForm(db: D1Database, teamId: number, lineup: string[]) {
  if (lineup.length !== 5) return { adr: null, kda: null };
  const latest = await db.prepare("SELECT MAX(played_at) latestAt FROM valorant_series").first<{ latestAt: string | null }>();
  if (!latest?.latestAt) return { adr: null, kda: null };
  const { results = [] } = await db.prepare(
    `SELECT s.played_at playedAt,p.adr,p.kills,p.deaths,p.assists FROM valorant_player_maps p JOIN valorant_maps m ON m.id=p.map_id JOIN valorant_series s ON s.id=m.series_id
     WHERE p.team_id=? AND p.player_name IN (${lineup.map(() => "?").join(",")})`,
  ).bind(teamId, ...lineup).all<{ playedAt: string; adr: number | null; kills: number | null; deaths: number | null; assists: number | null }>();
  const now = Date.parse(latest.latestAt.endsWith("Z") ? latest.latestAt : `${latest.latestAt.replace(" ", "T")}Z`);
  let adrTotal = 0, adrWeight = 0, kdaTotal = 0, kdaWeight = 0;
  for (const row of results) {
    ...
    if (row.adr !== null) { adrTotal += row.adr * weight; adrWeight += weight; }
    if (row.kills !== null && row.deaths !== null && row.assists !== null) { kdaTotal += ((row.kills + row.assists) / Math.max(1, row.deaths)) * weight; kdaWeight += weight; }
  }
  return { adr: adrWeight ? adrTotal / adrWeight : null, kda: kdaWeight ? kdaTotal / kdaWeight : null };
}
...
export default {
  async fetch(request: Request, env: Env) {
    ...
    if (url.pathname === "/api/valorant/matchup") {
      ...
      const [left, right, headToHead, meta, draftAStats, draftBStats, playerFormA, playerFormB, elo, lineupFormA, lineupFormB] = await Promise.all([
        ...
        lineupConfirmed ? valorantConfirmedLineupForm(env.DB, leftId, lineupA) : Promise.resolve({ adr: null, kda: null }), lineupConfirmed ? valorantConfirmedLineupForm(env.DB, rightId, lineupB) : Promise.resolve({ adr: null, kda: null }),
      ]);
      ...
      const lineup = lineupConfirmed && lineupFormA.adr !== null && lineupFormB.adr !== null && lineupFormA.kda !== null && lineupFormB.kda !== null ? { leftAdr: lineupFormA.adr, rightAdr: lineupFormB.adr, leftKda: lineupFormA.kda, rightKda: lineupFormB.kda, confirmed: true } : null;
